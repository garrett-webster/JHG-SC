...
def write_generational_results(theGenePools, popSize, gen, folder):
    ...
    if folder == "":
        output_dir = os.path.join(script_dir, "NoisyAgents", "attempt1") # just to give it somewhere to go
    else:
...
def run_jhg_stuff(jhg_engine, round, agents, numAgents):
    ...
    for transaction in transactions:
        noisy = noisify_transaction(transaction, noise_prob=0.15, noise_scale=0.10)
        noisy_transactions.append(noisy)


    # print("here is the transactions ", transactions)
    jhg_engine.play_round(noisy_transactions)

    return jhg_engine.get_influence()


def noisify_transaction(transaction, noise_prob=0.15, noise_scale=2.00):
    """Add small multiplicative noise to individual elements."""
    # Make a copy so we don't mess with the agent's internal state
    noisy = list(transaction)  # works for both lists and numpy arrays

    for i in range(len(noisy)):
        if random.random() < noise_prob:
            # Multiply by factor between (1-noise_scale) and (1+noise_scale)
            # e.g., 0.90x to 1.10x for noise_scale=0.10
            factor = random.uniform(1 - noise_scale, 1 + noise_scale)
            noisy[i] *= factor

    # Clip to valid range
    for i in range(len(noisy)):
        noisy[i] = max(-6, min(6, noisy[i]))
...
def playGame(theGene, agents, agentsPerGame, numExtraAgents, roundsPerGame, gen, game):
    ...
    pmetrics = getPmetrics(game, theGene, jhg_engine, agents, agentsPerGame, numExtraAgents, roundsPerGame)
...
def run_game(game_idx, gene, numGeneCopies, agentsPerGame, roundsPerGame, folder, extraAgents):
    ...
    # Run the game
    pmetrics = playGame(gene, agents, agentsPerGame, numExtraAgents, roundsPerGame, game_idx, folder)
...
def evolve_homogenous_JHG(popSize, numGeneCopies, startIndex, numGens, gamesPerGen, agentsPerGame,
...
if __name__ == "__main__":
    ...
    evolve_homogenous_JHG(popSize, numGeneCopies, startIndex, numGens, gamesPerGen, agentsPerGame, roundsPerGame, povertyLine, folder,
