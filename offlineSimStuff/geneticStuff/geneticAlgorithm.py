...
def playGame(agents, numPlayers, numRounds, gener, gamer, initialPopularities, povertyLine, forcedRandom):
    ...
    numToknes = 2 * numPlayers
    received = [0.0 for _ in range(numPlayers)]
    transactions = [0 for _ in range(numPlayers)]
    ...
    for r in range(numRounds):
        ...
        jhg_sim.apply_transaction(transactions)
    ...
    #jhg_sim.save(filename)

    transactions = None # looks different than the C++ code, thats becuase garbage collection is a jer.
    received = None
    jhg_sim = None
...
def mutateIt(gene):
    ...
    if (v >= 15): # 85 % chance of mutation within the mutaiton functiuon???
        return gene
    elif v < 3:
...
# ok so I do NOT understand what the final result it wants for this function is, at all. nice.
def evolvePopulationPairs(theGenePool_prev, popSize, numGeneCopies):
    ...
    num_genes = len(gene_keys) * 3 # somethign like that.
    ...
    print("TheGenePool_prev[0]->numGenes:", num_genes)

    theGenePool = [GeneAgent3("", numGeneCopies, 2) for _ in range(popSize)]
    ...
    for i in range(popSize):
        ...
        theGenePool[i] = GeneAgent3(geneStr, numGeneCopies, 2)
...
if __name__ == "__main__":
    ...
    agents = [AbstractAgent() for _ in range(popSize)] # litreally 0 clue on the best way to do this in python.
        ...
        for game in range(games_per_gen):
            ...
            for i in range(agentsPerGame):
                if theGenePools[plyrIdxs[i]].played_genes: # THANK GOODNESS I thought I was gonna have to assemble that from scratch.
                    ...
                    theGenePools[plyrIdxs[i]].absoluteFitness += ((pmetrics[i].avePop + pmetrics[i].endPop) / 2.0)
            ...
            theGenePools = evolvePopulationPairs(theGenePools_old, popSize, numGeneGopies)
