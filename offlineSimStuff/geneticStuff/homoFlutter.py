...
def write_generational_results(theGenePools, popSize, gen):
    ...
    avg_fitness = np.sum([agent.absoluteFitness for agent in theGenePools], axis=0) / popSize # go ahead and vectorize that fetcher
    avg_popularity = np.sum([agent.absolutePopularity for agent in theGenePools], axis=0) / popSize
