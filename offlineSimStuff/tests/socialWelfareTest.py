...
if __name__ == '__main__':
    ...
    for i in range(num_players):
        new_amount = 2 * num_players / (num_players - 1)
        new_allocations = [new_amount for _ in range(num_players)]
        new_allocations[i] = 0 # erase id
        T_one[i] = new_allocations
    ...
    # lets do like, 10 people
    T_two = [[] for _ in range(num_players)]

    for i in range(num_players):
        new_amount = 2 * num_players / (num_players)
        new_allocations = [new_amount for _ in range(num_players)]
        T_two[i] = new_allocations

    print("This is T_two ", T_two)
    ...
    if sum(new_pops) > sum(new_pops_two):
        print("no self is better")
    else:
        print("self is better")