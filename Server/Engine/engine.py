...
class JHGEngine():
    def __init__(self, alpha=0.2, beta=0.75, give=1.4, keep=1.0, steal=1.1, num_players=3, base_popularity=100.0,
                 poverty_line=0.0):
        # Scalars
    ...
    def I_hat(self, tau, t):
        ...
        Cs = np.zeros_like(w)
        with np.errstate(divide='ignore', invalid='ignore'):
            Cs = self.C_s * np.maximum(
                1 - (np.diag(T_) * w) / (np.ones((1, self.N)) @ (np.abs(T_minus) * w[:, np.newaxis])), 0)
            Cs[np.isnan(Cs)] = 0.0
            Cs[np.isinf(Cs)] = 0.0
