...
from pyqtgraph.examples.colorMapsLinearized import col_list, previous
import random
import copy
...
class optimalHuman:
    ...
    def get_vote(self, current_options_matrix, previous_votes=None, cycle=0, max_cycle=3):
        ...
        if self.allNegatives(current_options_matrix):
            ...
            self.get_vote_positive(our_row, col_probs, cause_sums, risk_aversion, majority_factor, current_options_matrix)
    ...
    def get_vote_negative(self, our_row, col_probs, cause_sums, risk_aversion, majority_factor, current_options_matrix, cycle, max_cycle):
        ...
        current_winner = max(cause_sums)
        new_cause_sums = copy.copy(cause_sums)
        new_cause_sums[new_cause_sums.index(current_winner)] = 0 # get rid of this
        second_most_likely_cause = max(new_cause_sums)
        if current_winner >= second_most_likely_cause * 2: # if there is no fetchign way
            return random.randrange(1, 3) # return a random causal vote
        else:
            if cycle == max_cycle-1:
                return 0 # final vote, so we are going to cast no vote and try to cause nothing to pass.
            else:
                return cause_sums.index(second_most_likely_cause) # still trying to posture to teh second most likely cause. 
